...
UPDATE "megadata_token" SET row_id = nextval('megadata_token_row_id_seq') 
WHERE row_id IS NULL AND id IN (
    SELECT id FROM "megadata_token" WHERE row_id IS NULL ORDER BY created_at
...
ALTER TABLE "megadata_token" ADD CONSTRAINT "megadata_token_row_id_pkey" PRIMARY KEY ("row_id");--> statement-breakpoint
ALTER TABLE "megadata_token" ALTER COLUMN "row_id" ADD GENERATED ALWAYS AS IDENTITY;--> statement-breakpoint
